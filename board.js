class Board {
  ...
  makeCellsClickable() {
    ...
    allCells.forEach((cell) => {
      cell.addEventListener("click", () => {
        ...
        const cellTypeRadio = Array.from(
          document.querySelectorAll("input[name=cellTypeRadio]")
        );
        ...
        if (newCellType === "A") {
          this.boardMatrix[this.pointA[0]][this.pointA[1]] =
            this.CELLTYPES["EMPTY"];
          this.pointA = [r_idx, c_idx];
        } else if (newCellType === "B") {
          this.boardMatrix[this.pointB[0]][this.pointB[1]] =
            this.CELLTYPES["EMPTY"];
          this.pointB = [r_idx, c_idx];
        } else if (newCellType === "WALL") {
          if (
            this.checkIfSameArrays(this.pointA, [r_idx, c_idx]) ||
            this.checkIfSameArrays(this.pointB, [r_idx, c_idx])
          ) {
  ...
  redrawBoard() {
    ...
    allCells.forEach((cell) => {
      ...
      cell.classList.remove(
        ...Array.from(cell.classList).filter(
          (className) => !["cell"].includes(className)
        )
      );
  ...
  iterate() {
    ...
    [this.boardMatrix, this.solved] = this.pathfinder.getBoardArray();
    if (this.solved) {
      clearInterval(this.pathfinding);
    }
    this.redrawBoard();
